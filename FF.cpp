...
pii find_max_allocatable_rack(vector<VM> & vms)
{
    vector<pii> candidate;
    //candidate.pb({inf, {1,1}}); // so that it cant be empty
...
pii find_max_allocatable_rack(vector<VM> & vms, int network)
{
    vector<pii> candidate;
...
pii find_max_allocatable_rack_among_candidates(vector<VM> & vms, vpii & candidates)
{
    vector<pii> candidate;
...
pii find_max_allocatable_rack_considering_partition(vector<VM> & vms, int pgindex, int p)
{
    vpii candidates =  possiblerack(pgindex, p);
    if(sz(candidates) == 0 )return {0,0};
    return find_max_allocatable_rack_among_candidates(vms, candidates);
}

//Given the network condition,
// If every rack doesn't satisfy the partition condition, we return {0,0}
pii find_max_allocatable_rack_considering_partition(vector<VM> & vms, int pgindex, int p, int network)
{
...
//4.update permutation so that we can maintain FF
void update_permutation(int network, int rack)
{
    if(nNuma==2)
    {
        vpii count;
        ...
        {
            int temp = 0;
            for(int j = 1 ; j<=nNuma; j++)
            {
                temp += leftover[network][rack][i][j].cpu;
                temp += leftover[network][rack][i][j].memory;
            }
            count.pb({temp, i});
        }
        sort(all(count));
        ...
        for(auto x: count)
        {
            int currentpm = x.se -1;
            if(nNuma==2)
            {
                permutation[i] = currentpm*2 +0;
                i++;
                permutation[i] = currentpm*2 +1;
                i++;
            }
        }
    ...
    {
        int n1 = 1; int n2 = (partner[1]==2)?3:2;

        vector<pair<int, pii>> count;

        for(int i=1 ; i<=nPm; i++)
        {
            int val1 = leftover[network][rack][i][n1].cpu + leftover[network][rack][i][n1].memory;
            val1 += leftover[network][rack][i][partner[n1]].cpu + leftover[network][rack][i][partner[n1]].memory;
            count.pb({val1,{i, n1}});
            int val2 = leftover[network][rack][i][n2].cpu + leftover[network][rack][i][n2].memory;
            val2 += leftover[network][rack][i][partner[n2]].cpu + leftover[network][rack][i][partner[n2]].memory;
            count.pb({val2,{i, n2}});
        ...
        for(auto x: count)
        {
            int pmindex = x.se.fi-1;
            int numaindex = x.se.se-1;
            permutation[i] = 4*pmindex + numaindex;
            i++;
            permutation[i] = 4*pmindex + partner[numaindex+1]-1;
            i++;
        }
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer)
{
    ...
    if(up)update_permutation(network_index, rack_index);
    up = false;
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer, int network)
{
    ...
    int rack_index = 1;
    if(up)update_permutation(network, rack_index);
    up = false;
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer, int network, int rack)
{
    ...
    // random starting index 
    //shuffle();
    if(up)update_permutation(network, rack);
    up = false;
...
bool cmp2(pii small, pii large)
{
    return small.se<large.se;
...
void allocate_with_constraint(vector<VM> & vms, vector<vector<int>> & answer, int pgindex, int p)
{
    ...
    //sort(all(possibleracks),cmp2);
    int m =sz(possibleracks);
    //int pointer = random(m)-1;
    int pointer = 0;
    if(up)update_permutation(possibleracks[pointer].fi, possibleracks[pointer].se);
    up = false;
...
{
    // A possible rack exists!
    vint order = find_optimal_network_order(vms);
    for (auto network : order)
    {
...
//if impossible, return {0,0}
pii find_first_allocatable_rack(vector<VM> & vms, int threshold, int network)
{
    for(int i=1; i<=nRack;i++)
    {
        if(maxallocate(vms, network,i) >= threshold)return {network, i};
    }
    return {0,0};
}


//choose the rack to place among given network, given candidate locations(for differentrack case)
pii choose_single_rack_among_candidates_same_network(vector<VM> & vms, int threshold, int network, vpii & locations)
{
    // if empty, false
    ...
    {
        if(maxallocate(vms, network, i.se)>=threshold)return{network,i.se};
    }
    return {0,0};
...
{
    if(orderof[x.fi] <orderof[y.fi])return true;
    if(orderof[x.fi] ==orderof[y.fi]&& x<=y)return true;
    return false;
...
{   
    ...
    if(differentrack ==false) 
    {
        ...
        {
            int network = find_optimal_network_order(vms)[0];
    ...
    else
    {
        ...
        REP0(i,n)
        {
            ...
            pii loc = choose_single_rack_among_candidates_same_network(vms, 1, network, candidates);
            if(loc != make_pair(0,0))
            {
                updatedanswer = true;
            ...
            if(updatedanswer==false)
            {
                ...
                pii loc = find_max_allocatable_rack_considering_partition(temp, pgindex, vms[i].partition, network);
                if(loc!=make_pair(0,0))
                {
...
bool hard_network_affinity_defined_allocate(vector<VM> & vms, vector<vint> & answer, int n, int network, bool differentrack, int pgindex)
{
    ...
    {
        //try single rack
        pii loc = find_first_allocatable_rack(vms, n, network);
        if(loc != make_pair(0,0) ){allocate_with_no_constraint(vms, answer, loc.fi, loc.se);return true;}
        //try single network
        int max_possible_allocation = maxallocate(vms, network);
    ...
    else if(differentrack==false)
    {
        ...
        pii loc = choose_single_rack_among_candidates_same_network(vms, n, network, candidates);
        if(loc!=make_pair(0,0)){allocate_with_no_constraint(vms, answer, loc.fi, loc.se);return true;}
                
        // If not, just try to choose from one rack that suffices the partition condition
        
        candidates =possiblerack(pgindex, vms[0].partition, network);
        loc = choose_single_rack_among_candidates_same_network(vms, n, network, candidates);
        if(loc!=make_pair(0,0)){allocate_with_no_constraint(vms, answer, loc.fi, loc.se);return true;}
        ...
        if(max_possible_allocation  >= n ){allocate_with_constraint(vms, answer, pgindex, vms[0].partition, network);return true;}
    ...
    else
    {
        REP0(i,n)
        {
            ...
            {
                vector<VM> temp = { vms[i]};
                pii loc = find_max_allocatable_rack_considering_partition(temp, pgindex, vms[i].partition);
                if(loc!=make_pair(0,0))
                {
                    if (maxallocate(temp,loc.fi, loc.se )>=1)
                    {
...
bool no_affinity_allocate(vector<VM> & vms, vector<vint> & answer, int n, bool differentrack, int pgindex)
{
    ...
    else if(differentrack==false)
    {
        ...
        // Choose the optimal among the candidates
        pii loc = choose_single_rack_among_candidates(vms, n, candidates);
        if(loc!=make_pair(0,0)){allocate_with_no_constraint(vms, answer, loc.fi, loc.se);return true;}
            
        // If not, just try to choose from one rack that suffices the partition condition
        candidates = possiblerack(pgindex, vms[0].partition);
        loc = choose_single_rack_among_candidates(vms, n, candidates);
        if(loc!=make_pair(0,0)){allocate_with_no_constraint(vms, answer, loc.fi, loc.se);return true;}
        ...
        if(max_possible_allocation  >= n ){allocate_with_constraint(vms, answer, pgindex, vms[0].partition);return true;}
