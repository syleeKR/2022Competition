...
///////////////////////////////////////////Todo//////////////////////////////////////////////
// numacount pair(1,2) 만 현재 사용하고 있음
// network random choosing 하고있음.
    ...
    else
    {
        for(auto x : candidate)
        {
            for(int pm_index = 1; pm_index <=nPm; pm_index++)
            {
                ...
                {
                        int leftovercpu1 = leftover[x.fi][x.se][pm_index][numa_index*2-1].cpu;
                        int leftovermemory1 = leftover[x.fi][x.se][pm_index][numa_index*2-1].memory;

                        int leftovercpu2 = leftover[x.fi][x.se][pm_index][numa_index*2].cpu;
                        int leftovermemory2 = leftover[x.fi][x.se][pm_index][numa_index*2].memory;
...
pii find_min_rack_considering_partition(vector<VM> & vms, int pgindex, int p)
{
    ...
    return find_min_rack_among_candidates(vms, candidates);
}
...
bool hard_rack_affinity_allocate(vector<VM> & vms, vector<vint> & answer, int n)
{
    ...
    if(max_possible_allocation < n)return false;
    // if possible
...
{   
    ...
    else
    {
        ...
        REP0(i,n)
        {
            ...
            if(updatedanswer==false)return false;
        }
        return true;
    }
