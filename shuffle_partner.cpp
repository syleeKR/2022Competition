...
void preprocess()
{
    permutation.resize(nNuma * nPm);
    REP0(i, nNuma * nPm)permutation[i] = i;
}
void shuffle()
{
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer)
{
    ...
    while(toassign < n)
    {
        ...
        while( canallocate(vms[toassign],network_index, rack_index, pm_index, numa_index))
        {
            ...
            if(type==1)numa_index = partner[numa_index];
            toassign ++;
            if(toassign==n)break;
        }
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer, int network)
{
    ...
    {            
        ...
        if( type==1 && canallocate(vms[toassign],network, rack_index, pm_index, partner[numa_index]))
        {
            update(vms[toassign],network, rack_index, pm_index, partner[numa_index]);
            update_vector(answer, toassign, network, rack_index, pm_index, partner[numa_index]);
            toassign ++;
            if(toassign==n)break;
        }
...
void allocate_with_no_constraint(vector<VM> & vms, vector<vector<int>> & answer, int network, int rack)
{
    ...
    {        
        ...
        if( type==1 && canallocate(vms[toassign],network, rack, pm_index, partner[numa_index]))
        {
            update(vms[toassign],network, rack, pm_index, partner[numa_index]);
            update_vector(answer, toassign, network, rack, pm_index, partner[numa_index]);
...
void allocate_with_constraint(vector<VM> & vms, vector<vector<int>> & answer, int pgindex, int p, int network)
{
    ...
    while(toassign < n)
    {
        ...
        if( canallocate(vms[toassign],possibleracks[pointer].fi,possibleracks[pointer].se, pm_index, partner[numa_index]))
        {
            update(vms[toassign],possibleracks[pointer].fi,possibleracks[pointer].se, pm_index, partner[numa_index]);
            update_vector(answer, toassign, network, possibleracks[pointer].se,pm_index, partner[numa_index] );
            toassign ++;
            if(toassign==n)break;
        }
        // update the three indices
